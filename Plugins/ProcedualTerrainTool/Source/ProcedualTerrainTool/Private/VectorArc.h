 ...
 */
USTRUCT()
struct PROCEDUALTERRAINTOOL_API FVectorArc
{
	GENERATED_BODY()
	...
	/**
	 * Gets the inverse of this Arc.
	 * 
	 * @return An arc that contains only the area not contained by this arc.
	 */
	FVectorArc Inverse() const
	{
		return FVectorArc(GetUpperBound(), GetLowerBound());
	}
	...
	/**
	 * Gets whether or not the given arc is between the LowerBound and UpperBound of this.
	 * 
	 * @param Arc - The arc to check.
	 * @param bInclusiveBounds - Whether or not to count a location on the bounds as between them.
	 * @return Whether or not the given arc is between the LowerBound and UpperBound of this.
	 */
	bool IsArcContainedInArc(FVectorArc Arc, bool bInclusiveBounds = true) const
	{
		return (IsLocationInArc(Arc.GetUpperBound(), bInclusiveBounds) && IsLocationInArc(Arc.GetLowerBound(), bInclusiveBounds));
	}
